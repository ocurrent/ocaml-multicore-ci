...
let remove_dev_selections_from_opam_build ~opam_files selections =
  selections |> List.map (fun sel -> List.fold_left (fun acc opam_file ->
...
module Analysis = struct
  ...
  let sandmark_dummy_analysis ~platforms ~opam_repository_commits ~package_name =
    { opam_files = [];
      ocamlformat_source = None;
      selections = `Sand_build (
        package_name,
        make_placeholder_selections ~platforms ~opam_repository_commits
      );
    }
  ...
  let solve ~root_pkgs ~pinned_pkgs ~platforms ~opam_repository_commits ~job ~solver =
    let platforms = List.map (fun (variant,vars) -> Variant.to_string variant, vars) platforms in
    let opam_commits_as_strs = opam_repository_commits |> List.map (fun commit ->
      Current_git.Commit_id.(repo commit, hash commit)
    ) in
    let request =
      { Ocaml_multicore_ci_api.Worker.Solve_request.
        opam_repository_commits = opam_commits_as_strs;
        root_pkgs;
        pinned_pkgs;
        platforms
      }
  ...
  let of_dir ~solver ~job ~platforms ~opam_repository_commits ~package_name ?is_compiler ?compiler_commit dir =
    ...
    if is_compiler || opam_files = [] then Lwt_result.return (
      ...
      let selections = match compiler_commit with
      | None -> selections
...
module Examine = struct
  ...
  let run solver job { Key.src; _ } { Value.opam_repository_commits; platforms; is_compiler; compiler_commit; sandmark_package } =
    ...
    | Some package_name ->
      Current.Job.log job "The analysis is skip, %s sandmark package dependency does not need to be analysed" package_name;
      Lwt_result.return (Analysis.sandmark_dummy_analysis ~platforms ~opam_repository_commits ~package_name)
