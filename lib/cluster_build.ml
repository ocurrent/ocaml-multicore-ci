...
module Op = struct
  ...
  module Key = struct
    type t = {
      ...
      sandmark_package : string option;
          (*A package name which we want to build or install, this is helpful if we want to use this build for a specific package instead of the repo*)
    }
        ...
        [
          ...
          ( "compiler_commit",
            ...
            | Some compiler_commit ->
                `String (Current_git.Commit_id.hash compiler_commit) );
          ("repo", `String repo);
          ( "test_repo",
            match test_repo with None -> `Null | Some r -> `String r );
          ("label", `String label);
          ( "sandmark_package",
            match sandmark_package with None -> `Null | Some p -> `String p );
        ]
  ...
  let get_cache_hint repo { Value.base; variant; ty } =
    Fmt.str "%s-%s-%a-%s" repo (Image.hash base) Variant.pp variant
      (Spec.digest_of_ty ~variant ty)

  let run t job
      { Key.pool; commit; compiler_commit; label = _; repo; test_repo; _ } spec
      =
    let { Value.base; variant; ty } = spec in
    let build_spec =
      Build.make_build_spec ~base ~compiler_commit ~repo ~test_repo ~variant ~ty
    in
    ...
    Current.Job.write job
      (Fmt.str
         ...
         (Obuilder_spec.Docker.dockerfile_of_spec ~os:`Unix ~buildkit:false
            build_spec));
    let spec_str = Fmt.to_to_string Obuilder_spec.pp build_spec in
    let action = Cluster_api.Submission.obuilder_build spec_str in
    let src =
      ( Git.Commit_id.repo build_context_commit,
        [ Git.Commit_id.hash build_context_commit ] )
    in
  ...
  let pp f
    ...
    Fmt.pf f "test %a %s %a %a %a (%s:%s)"
      Fmt.(option string)
      sandmark_package repo (Fmt.option Fmt.string) test_repo
      Current_git.Commit_id.pp commit
      (Fmt.option Current_git.Commit_id.pp)
      compiler_commit pool label
...
let build t ~platforms ~spec ~repo ?test_repo ?compiler_commit ?sandmark_package
    commit =
...
let v t ~platforms ~repo ?test_repo ?compiler_commit ?sandmark_package ~spec
    source =
  let build =
    build t ~platforms ~spec ~repo ?test_repo ?compiler_commit ?sandmark_package
      source
  in
