(** The index is:
    ...
    - A map from project builds ([owner * name * hash * variant)] tuples) to
      statuses.
...
val record :
  ...
  unit
(** [record ~owner ~name ~hash ~variant job] updates the entry for
    [owner, name, hash, variant] to point at job. *)

val remove :
  owner:string -> name:string -> hash:string -> variant:string -> unit
(** [remove ~owner ~name ~hash ~variant] removes the entry for
...
val get_all_jobs :
  unit -> (string * string * string * string * string * job_state) list
(** [get_all_jobs ()] is the last known set of OCurrent jobs. The result is
    (owner, name, hash, job_id, variant, job_state). *)
