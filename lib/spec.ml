...
type ty = [
  | `Opam of [ `Build | `Lint of [ `Doc | `Opam ] | `Make of string list | `Script of string list] * Selection.t * opam_files
...
let opam ~label ~selection ~analysis op =
  ...
  let ty =
    match op with
    | `Build | `Lint (`Doc | `Opam) | `Make _ | `Script _ as x -> `Opam (x, selection, Analyse.Analysis.opam_files analysis)
...
let success_result spec =
  success_result_of_ty spec.ty
