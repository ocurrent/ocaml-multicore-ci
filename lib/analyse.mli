...
val examine_with_compiler :
  ...
  Current_git.Commit.t Current.t ->
  Analysis.t Current.t
(** Similar to [examine] except with the differing assumption that we will be
    compiling using the specified compiler_commit. This means that we won't
    build under all platforms, we'll just pick one to suit the compiler. It also
    means that we're not going to pin local opam files, but we'll build from the
    published opam-repository instead. *)
