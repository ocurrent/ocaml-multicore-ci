...
let rec with_commit_lock ~job commit variant fn =
  ...
  | Some lock ->
      ...
      lock >>= fun () -> with_commit_lock ~job commit variant fn
  | None ->
...
let make_build_spec ~base ~repo ~test_repo ~compiler_commit ~variant ~ty =
  ...
  | `Opam (`Build, selection, opam_files) -> (
      ...
      | None ->
          Opam_build.spec_dune ~repo:test_repo ~base ~opam_files
            ~compiler_commit ~selection
      | Some _ ->
          Opam_build.spec_opam_install ~base ~opam_files ~compiler_commit
            ~selection)
  | `Opam (`Lint `Doc, selection, opam_files) ->
      Lint.doc_spec ~base ~opam_files ~selection
  | `Opam (`Lint `Opam, selection, opam_files) ->
      Lint.opam_lint_spec ~base ~opam_files ~selection
  | `Opam (`Make targets, selection, opam_files) ->
      Opam_build.spec_make ~repo:test_repo ~base ~opam_files ~compiler_commit
        ~selection ~targets
  | `Opam (`Script cmds, selection, opam_files) ->
      Opam_build.spec_script ~repo:test_repo ~base ~opam_files ~compiler_commit
        ~selection ~cmds
...
module Op = struct
  ...
  module Key = struct
    type t = {
      ...
      compiler_commit : Current_git.Commit.t option;
        ...
        [
          ...
          ( "compiler_commit",
            ...
            | Some compiler_commit ->
                `String (Current_git.Commit.marshal compiler_commit) );
          ("repo", `String repo);
          ( "test_repo",
            match test_repo with None -> `Null | Some r -> `String r );
          ("label", `String label);
        ]
  ...
  let run { Builder.docker_context; pool; build_timeout } job
      { Key.commit; compiler_commit; label = _; repo; test_repo }
      { Value.base; variant; ty } =
    let compiler_commit_id = Option.map Current_git.Commit.id compiler_commit in
    let build_context_commit =
      match compiler_commit with None -> commit | Some cc -> cc
    in
    let build_spec =
      make_build_spec ~base ~compiler_commit:compiler_commit_id ~repo ~test_repo
        ~variant ~ty
    in
    let make_dockerfile ~for_user =
      (if for_user then "" else Buildkit_syntax.add (Variant.arch variant))
      ^ Obuilder_spec.Docker.dockerfile_of_spec ~os:`Unix
          ~buildkit:(not for_user) build_spec
    in
    ...
    Current.Job.write job
      (Fmt.str
         ...
         (Current_git.Commit.id build_context_commit)
    ...
    Current_git.with_checkout ~pool:checkout_pool ~job build_context_commit
    @@ fun dir ->
    Current.Job.write job
  ...
  let pp f ({ Key.repo; test_repo; commit; compiler_commit; label }, _) =
    Fmt.pf f "test %s %a %a %a (%s)" repo (Fmt.option Fmt.string) test_repo
      Current_git.Commit.pp commit
      (Fmt.option Current_git.Commit.pp)
      compiler_commit label
...
let build ~platforms ~spec ~repo ?test_repo ?compiler_commit commit =
  ...
  |> let> { Spec.variant; ty; label } = spec
     ...
     | Some { Platform.builder; variant; base; _ } ->
         BC.run builder
           { Op.Key.commit; compiler_commit; repo; test_repo; label }
...
let v ~platforms ~repo ?test_repo ?compiler_commit ~spec source =
  ...
  and+ spec = spec in
  let result = state |> Result.map (fun () -> Spec.success_result spec) in
  (result, job_id)
