...
let repos =
  ...
  & info
      ~doc:
        "The directory containing the .git subdirectory. May be specified \
         multiple times."
      ~docv:"DIR" []
...
let cmd =
  ...
  let term =
    Term.(
      term_result
        (const main $ Current.Config.cmdliner $ Current_web.cmdliner $ repos))
  in
