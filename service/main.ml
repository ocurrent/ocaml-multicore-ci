...
let has_role user = function
  ...
  | _ -> (
      ...
      | Some
          ( "github:talex5" | "github:avsm" | "github:ewanmellor"
          | "github:kit-ty-kate" | "github:samoht" | "github:moyodiallo"
          | "github:tmcgilchrist" | "github:mtelvers" ) ->
          true
      | _ -> false)

let main config mode app capnp_address github_auth submission_uri solver_uri :
  ...
  Lwt_main.run
    ( run_capnp capnp_address >>= fun (vat, rpc_engine_resolver) ->
      ...
      let engine =
        Current.Engine.create ~config (Pipeline.v ?ocluster ~solver ~confs)
      in
      rpc_engine_resolver
      |> Option.iter (fun r ->
      ...
      let routes =
        Routes.(
          ...
          @--> Current_github.webhook ~engine
                 ~get_job_ids:Ocaml_multicore_ci.Index.get_job_ids
                 ~webhook_secret)
        :: Routes.(
      ...
      let site =
        Current_web.Site.v ?authn ~has_role ~secure_cookies
          ~name:"ocaml-multicore-ci" routes
      in
...
let solver_service =
  Arg.value
  @@ Arg.opt Arg.(some Capnp_rpc_unix.sturdy_uri) None
  @@ Arg.info ~doc:"The solver.cap file for the solver scheduler service"
       ~docv:"FILE" [ "solver-service" ]

let github_app_id =
  Arg.required @@ Arg.opt Arg.(some string) None @@ Arg.info [ "github-app-id" ]
...
let () =
  let with_github =
    match Cmd.eval_peek_opts github_app_id with
    | None, _ -> false
    | Some _, _ -> true
  in
