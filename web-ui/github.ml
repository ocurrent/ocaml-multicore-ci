...
let stream_logs job ~owner ~name ~refs ~hash ~jobs ~variant ~status (data, next)
  ...
  let rec aux next =
    ...
    | Ok (data, next) ->
        Transfer_IO.write writer (Ansi.process ansi data) >>= fun () -> aux next
    | Error (`Capnp ex) ->
...
let repo_handle ~meth ~owner ~name ~repo path =
  ...
  | `GET, [ "commit"; hash; "variant"; variant ] ->
      ...
      let write _ic oc =
        ...
        Lwt.finalize
          (fun () ->
            stream_logs job ~owner ~name ~refs ~hash ~jobs ~variant ~status
              chunk writer
  ...
  | `POST, [ "commit"; hash; "variant"; variant; "rebuild" ] -> (
      ...
      match Capability.problem new_job with
      | None ->
