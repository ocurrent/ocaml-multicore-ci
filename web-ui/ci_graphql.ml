...
type job_state = NotStarted | Passed | Failed | Active | Aborted | Undefined

module Config = struct
  type t = { admin_service_uri : string }
end

module Org = struct
  type t = { org : Client.Org.t; name : string }
end

module Repo_info = struct
  type t = { repo : Client.Repo.t; name : string; master_status : build_status }
end
...
let remap_capnp_err result =
  Lwt_result.map_error
    (function `Capnp err -> Fmt.to_to_string Capnp_rpc.Error.pp err)
    result
...
let list_all_jobs ci : (Job_info.t list, string) Lwt_result.t =
  ...
  |> lwt_result_map_list (fun job ->
         ...
         let error = error_of_job_state orig_outcome in
         {
           Job_info.owner = job.Client.owner;
           name = job.Client.name;
           hash = job.Client.hash;
           job_id = job.Client.job_id;
           variant = job.Client.variant;
           outcome;
           error;
         })
...
let list_repos org : (Repo_info.t list, string) Lwt_result.t =
  ...
  |> lwt_result_map_list (fun repo_info ->
         ...
         {
           Repo_info.repo;
           name = repo_info.Client.Org.name;
           master_status = repo_info.Client.Org.master_status;
         })
...
let build_status =
  Ocaml_multicore_ci_api.Raw.Build.Reader.BuildStatus.(
    Schema.(
      enum "build_status"
        ~values:
          [
