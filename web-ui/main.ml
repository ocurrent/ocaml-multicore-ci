...
let handle_request ~backend ~docroot ~admin_service_uri conn request body =
  ...
  | `OPTIONS, "graphql" :: _ ->
      Server.respond_string ~status:`OK ~headers:response_headers ~body:"" ()
      |> normal_response
  | _, "graphql" :: _ -> (
      ...
      let graphql_callback =
        Graphql_cohttp_lwt.make_callback
          (fun _req -> ())
          (Ci_graphql.schema ~admin_service_uri ci)
      in
      ...
      | `Response (resp, body) ->
          let resp_with_cors =
            make ~version:resp.version ~status:resp.status ~flush:resp.flush
  ...
  | meth, "github" :: path when path != [] -> Github.handle ~backend ~meth path
...
let main port backend_uri admin_service_uri docroot prometheus_config =
  Lwt_main.run
    (let admin_service_uri =
       match String.cut ~sep:"/" ~rev:true (String.trim admin_service_uri) with
       | Some (uri, "") -> uri
       | _ -> admin_service_uri
     in
     ...
     let config =
       Server.make_response_action
         ~callback:(handle_request ~backend ~docroot ~admin_service_uri)
         ()
     in
...
let admin_service_uri =
  ...
  @@ Arg.info
       ~doc:
         "The public endpoint for the admin UI. This is used when linking from \
          this frontend to the admin UI."
       ~docv:"URI" [ "admin-service-uri" ]
...
let docroot =
  ...
  @@ Arg.info
       ~doc:
         "The docroot for the web UI. This is used to find the built \
          JavaScript and images etc."
       ~docv:"PATH" [ "docroot" ]
